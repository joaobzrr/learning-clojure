...
-   **Files and Directories on Disk**: Use `snake_case` (underscores) for all file and directory names (e.g., `my_new_exercise`, `my_new_exercise_test.clj`). This is a requirement for the Clojure runtime to be able to map namespaces to files correctly.
-   **Clojure Namespaces**: Use `kebab-case` (dashes) for Clojure namespaces (e.g., `exercises.my-new-exercise.solution`).
...
2.  `src/exercises/<exercise_name>/solution.clj`: A Clojure file. The namespace should follow the pattern `exercises.<exercise-name>.solution`. This file should contain a minimal stub implementation that allows the tests to compile and run, but fails the tests (e.g., returns nil, empty collections, or incorrect values), so students can see failing tests and then implement the correct solution.
3.  `test/exercises/<exercise_name>_test.clj`: A Clojure file containing the tests for the exercise. The namespace for this test file must be `exercises.<exercise-name>-test`.
...
1.  **Create a new directory** for the exercise under `src/exercises/`. The directory name must use `snake_case` (e.g., `src/exercises/my_new_exercise`).
2.  **Create the exercise files**:
    *   Inside the new directory, create a `README.md` file with the exercise instructions.
    *   Create a `solution.clj` file. Its namespace must be `exercises.<exercise-name>.solution` (e.g., `(ns exercises.my-new-exercise.solution)`). This file should contain a minimal stub implementation that allows the tests to compile and run, but fails the tests (e.g., returns nil, empty collections, or incorrect values).
3.  **Create a test file**:
    *   Create a test file named `<exercise_name>_test.clj` directly under `test/exercises/` (e.g., `test/exercises/my_new_exercise_test.clj`).
    *   Its namespace must be `exercises.<exercise-name>-test` (e.g., `(ns exercises.my-new-exercise-test)`).
...
1.  Come up with a concept or topic for the exercise.
2.  Create a new exercise following the structure and naming conventions defined in "How to Add a New Exercise".
3.  The `README.md` should clearly explain the problem and the tasks.
